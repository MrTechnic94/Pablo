...
async function embedUpdater(client, logger) {
    ...
    for (const member of channel.guild.members.cache.values()) {
        if (member.presence && [PresenceUpdateStatus.Online, PresenceUpdateStatus.Idle, PresenceUpdateStatus.DoNotDisturb].includes(member.presence.status)) onlineCount++;
    }
