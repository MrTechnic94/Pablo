        ...
        outdated.on('close', () => {
            ...
            update.on('close', (code) => {
                if (code === 0) {
                    ...
                    const hasLintPackage = (pkg.dependencies?.['@biomejs/biome'] || (pkg.optionalDependencies?.['@biomejs/biome'] || (pkg.devDependencies?.['@biomejs/biome'])));

                    if (hasLintPackage) {
                        const lintUpdate = spawn('pnpm', ['lint:update'], { cwd: process.cwd(), stdio: 'inherit' });

                        lintUpdate.on('close', (lintCode) => {
                            if (lintCode === 0) {
                            ...
                            logger.info('[Client] Restarting the bot...');
                            process.exit(0);
                        });

                        lintUpdate.on('error', (err) => {
